...
async function getAllConfigTrojan(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const trojanTls = `trojan://bexnxx\u0040${hostName}:443?security=tls&type=ws&host=${hostName}&sni=${hostName}&fp=random&path=${pathFixed}#${data.isp} (${data.country})`;
        const trojanNtls = `trojan://bexnxx\u0040${hostName}:80?security=none&type=ws&host=${hostName}&path=${pathFixed}#${data.isp} (${data.country})`;
